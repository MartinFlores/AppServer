...
public class ClientHandler implements Runnable {
    ...
    private void sendError404(PrintWriter writer, String path) {
        ...
        if (path != null && path.startsWith("/app")) {
            String msg = "No se encontró la webapp. Descomprima 'webapp.zip' en la carpeta 'www' (servida en '/app').";
            writer.print("<script>alert(\"" + msg + "\");</script>");
        }

        writer.print("<h1>Error V10: Archivo no encontrado</h1>");
        writer.print("<p>Ruta: " + path + "</p>");
        writer.print("<p>IP: " + NetworkUtils.getLocalIp() + "</p>");
        writer.flush();
    }
}
