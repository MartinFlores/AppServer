...
public class DBSchema {
   ...
   public static String[] getCreateTableStatements() {
      return new String[] {
            ...
            "CREATE TABLE IF NOT EXISTS products (id INTEGER PRIMARY KEY AUTOINCREMENT, category_id INTEGER, name TEXT NOT NULL, description TEXT, price REAL NOT NULL, purchase_price REAL, stock INTEGER DEFAULT 0, image TEXT, status TEXT DEFAULT 'available', created_at INTEGER NOT NULL, FOREIGN KEY(category_id) REFERENCES categories(id))",
   ...
   public static String[] getDefaultDataStatements() {
      ...
      return new String[] {
            "INSERT OR IGNORE INTO company_config (id, company_name, username, pin, primary_color, updated_at) VALUES (1, '', '', '', '#FF6B35', "
                  + now + ")",
            "INSERT OR IGNORE INTO categories (id, name, icon, created_at) VALUES (1, 'Hamburguesas', '\ud83c\udf54', "
                  + now + ")",
            "INSERT OR IGNORE INTO categories (id, name, icon, created_at) VALUES (2, 'Pizzas', '\ud83c\udf55', " + now
                  + ")",
            "INSERT OR IGNORE INTO categories (id, name, icon, created_at) VALUES (3, 'Bebidas', '\ud83e\udd64', " + now
                  + ")",
            "INSERT OR IGNORE INTO categories (id, name, icon, created_at) VALUES (4, 'Postres', '\ud83c\udf70', " + now
                  + ")",
            "INSERT OR IGNORE INTO products (id, category_id, name, description, price, status, created_at) VALUES (1, 1, 'Classic Burger', 'Carne res, queso, lechuga', 85.0, 'available', "
                  + now + ")",
            "INSERT OR IGNORE INTO products (id, category_id, name, description, price, status, created_at) VALUES (2, 1, 'Mega Bacon', 'Doble carne, extra tocino', 120.0, 'available', "
                  + now + ")",
            "INSERT OR IGNORE INTO products (id, category_id, name, description, price, status, created_at) VALUES (3, 2, 'Pepperoni Classic', 'Mucha mozzarella y pepperoni', 150.0, 'available', "
                  + now + ")",
            "INSERT OR IGNORE INTO products (id, category_id, name, description, price, status, created_at) VALUES (4, 3, 'Coca Cola 600ml', 'Bien fría', 25.0, 'available', "
                  + now + ")",
            "INSERT OR IGNORE INTO products (id, category_id, name, description, price, status, created_at) VALUES (5, 4, 'Cheesecake', 'Frutos rojos', 45.0, 'available', "
                  + now + ")" };
   }
}
