...
const saveCategory = () => {
  ...
  if (editingId.value !== null) {
    // Mock update in store
    const cat = saleStore.categories.find((c) => c.id === editingId.value)
    if (cat) {
    ...
    // Mock add to store
    const newId = Math.max(0, ...saleStore.categories.map((c) => c.id)) + 1
    saleStore.categories.push({
      id: newId,
...
const deleteCategory = (id: number) => {
  const index = saleStore.categories.findIndex((c) => c.id === id)
  if (index !== -1) {
    saleStore.categories.splice(index, 1)
  }
}
...
<template>
  ...
  >
    <div v-if="show" class="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-4">
      <!-- Backdrop -->
      ...
      >
        ...
        <div
          ...
          class="p-6 bg-slate-50 border-b border-slate-100 animate-in slide-in-from-top duration-300"
        >
          <div class="space-y-4">
